...
| Radio | On-board **ANT1**, shared 2.4 GHz Wi-Fi / BLE. On a physical unit: embassy-debug `--features radio` printed `wifi n=` and `ble n=` in one listen ([pin-map.md](references/pin-map.md#on-a-physical-unit-embassy-debug-radio-feature)). embassy-debug `--features pair` exists; pairing is **not measured** |
