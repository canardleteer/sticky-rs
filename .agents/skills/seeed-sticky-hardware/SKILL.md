...
| Radio | On-board **ANT1**, shared 2.4 GHz Wi-Fi / BLE. On a physical unit: embassy-debug `--features radio` printed `wifi n=` and `ble n=` in one listen ([pin-map.md](references/pin-map.md#on-a-physical-unit-embassy-debug-radio-feature)). Default `pair` advertised `sticky-rs` only on that card; a host BlueZ Connect typed UART `pair pin=` and completed SMP (`pair ok`, pair card `Paired`). An earlier `--features pair` sit stopped at `pair fail=pairing`. Default image paints idle-until-touch Wi-Fi survey / WPA2 SoftAP cards (`sticky-rs-AP`); a first sit walked `scene=wifi_survey` / `scene=wifi_ap` but START missed until hit-test used the gray4 canvas. Landscape0 START uses `gray4_touch_framebuffer` (OTP 180° only). 2026-09-04 `wifi_ap` sit: `page=362,53 hit=1` / `p0=362,426` then `state=active`; the OR image also toggled the empty opposite side. SoftAP join / `GET /` is **host-verified** (2026-09-04; spare STA, DHCP `192.168.4.50`, JSON `device` / `scene=wifi_ap` / `wifi` counts). After STOP + replug + START: UART `wifi_ap … clients=1` then `wifi_http req=1 path=/`. Host disconnect produced no later `wifi_ap` decrement on that image. Later operator glass `clients=0` `http=2` after a host leave (no UART). |
