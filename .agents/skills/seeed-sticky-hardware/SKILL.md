...
| Touch | **GT911** on its own I2C; sensor reports **480×800** (portrait); UART `p0=` is `to_screen` (glass, after the panel 180°). IMU-page hit-test uses `to_framebuffer` + `framebuffer_to_page` (gray4 canvas). **5** simultaneous contacts on this FPC (Rev.09 §1). INT low at RST → `0x5D` (Rev.09 §6.1; [touch.md](references/touch.md#on-a-physical-unit-embassy-debug)). Wi-Fi START sit: [touch.md](references/touch.md#coordinate-transform-on-a-physical-unit) |
