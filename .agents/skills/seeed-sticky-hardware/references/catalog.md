...
| ESPHome `seeed-reterminal-sticky` | YAML / Playground | **Not in the registry.** Playground generates YAML; the driver is ESPHome `epaper_spi` SSD1677 ([PR 16950](https://github.com/esphome/esphome/pull/16950)). B/W only (`EPaperMono`), booster Level 2, `0x22` F7/FF, sleep `0x03`. The official example omits touch, IMU, SD, mic, and deep sleep. Orientation may not match Bunny glass mapping. [sira-fiinikkusu/reterminal-sticky-voice-companion](https://github.com/sira-fiinikkusu/reterminal-sticky-voice-companion) is household ESPHome on this stack: PDM mic (16 kHz left, GPIO19/20, EN 38), USB-Serial-JTAG pad release after deep sleep, and sleep YAML. Wiring: [sensors.md](sensors.md#pdm-microphone). |
| FreeInk `STICKY` / CrossPoint | Compiled board profile | Wiring intent for mic/SHT40/GPIO40. Default 40 MHz SPI and `n16r8` 16 MB limits are wrong for this flash. MCU gray LUT is **not** factory data. |
