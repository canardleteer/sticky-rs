...
Passive buzzer on **GPIO48**. PWM (LEDC) works; hold low in deep sleep. Part
and resonance: [nyc-buzzer-part](../resources/not-yet-confirmed.md#nyc-buzzer-part).
A ~1 kHz carrier has been used as a volume-via-duty drive.
...
| Card detect | 11 | Pull-up; interrupt- and wake-capable; polarity: [nyc-sd-detect](../resources/not-yet-confirmed.md#nyc-sd-detect) |
...
Card voltage, max capacity, and detect polarity:
[nyc-sd-detect](../resources/not-yet-confirmed.md#nyc-sd-detect). Factory
firmware ACKs the slot and hotplug; flash LittleFS on the factory image is
**not** the SD card.
...
with no edges. That does not close
[nyc-sd-detect](../resources/not-yet-confirmed.md#nyc-sd-detect).
...
`vbus=1` again. PD negotiation:
[nyc-usb-pd](../resources/not-yet-confirmed.md#nyc-usb-pd).
