...
| GT911 glass corners | 2026-08-30. USB-C down, finger-pad taps near the ink corners (in one sample). Old `to_screen` (sample as 800×480): keys-side `p0=` `792,195` / `0,195`; mid left→right Y only `477→199`. After map from 480×800: first `p0=` `795,470`, `795,4`, `4,475`, `4,4`. Closes `nyc-gt911-corners`. |
| IMU page vs USB-C | 2026-08-30. embassy-debug splash, glass facing operator. Floor → `imu=Portrait0` readable; ceiling → `Portrait180` readable. Identity landscape was RTL; after `page_to_framebuffer` mirror X, right → `Landscape0` readable and left → `Landscape180` readable. Tokens match. Closes `nyc-page-rotation`. |
| Wi-Fi START hit-test | 2026-09-04. Default embassy-debug walk printed `scene=wifi_survey` / `scene=wifi_ap`. START taps (`p0=679,189` while `imu=Portrait0`) printed `touch n=1` but no `wifi_survey` / `wifi_ap` when hit-test used UART `to_screen` as the gray4 canvas. Undo the transmit 180° (`to_framebuffer` + `framebuffer_to_page`) and that tap is the START button. Same day, landscape OR image: FaceUp (last landscape page) on `scene=wifi_ap` printed `wifi tap page=362,53 hit=1` / `p0=362,426` then `wifi_ap state=active` (canvas page is the top strip; glass `p0=` is the bottom — OTP 180). STOP `page=382,78 hit=1` / `p0=382,401` then `state=stopped`. Operator then toggled from the empty opposite side (OR of both canvases). Hit-test is now both landscape holds = OTP 180 only. Same evening 2026-09-04, Landscape180 `wifi_survey` on the tap-canvas image: opposite `page=435,53 hit=0` / `p0=364,53`; canvas `page=452,399 hit=1` / `p0=347,399`; UART `wifi_survey count=16` then `count=18` (that canvas path is the USB-C-right strip). 2026-09-05 USB-C left, OTP-180 image (operator confirmed): survey visible `page=302,408 hit=1` / `p0=302,71` then `count=21`; SoftAP visible START `page=339,402 hit=1` / `p0=339,77` then `wifi_ap state=active`; STOP `page=323,433 hit=1` / `p0=323,46`; ghost `page=341,70 hit=0` / `p0=341,409`. Each START / result is a full OTP gray4 (whole panel). Does not close SoftAP leave-drop. |
| SoftAP DHCP + JSON | 2026-09-04. After the hit-test fix, a host spare STA joined `sticky-rs-AP` / `sticky26` (WPA2). First sits (no CH343): DHCP `192.168.4.50`, `GET /` JSON `device` / `scene=wifi_ap` / `wifi` counts (`requests` 1 then 2). After STOP + replug + START: UART `wifi_ap state=active … clients=1`, then `wifi_http req=1 path=/`. Same DHCP / JSON `clients=1` `requests=1`. Host `nmcli` disconnect: no later `wifi_ap` decrement in a ~40 s listen. Glass still `clients=1` after that disconnect (operator, same sit). Same evening, landscape-button image, no UART (USB-C in other use): spare STA, DHCP `192.168.4.50`, `GET /` `clients=1` `requests=1` then `requests=2`. After host disconnect, operator glass `clients=0` `http=2` (leave-drop + HTTP count kept). |
| Charge `/CE` pulse | 2026-08-30. embassy-debug `--features charge`, USB present. First sit: `ce parked gpio40=1 vbus=1 i=0`, `ce on gpio40=0 i=0`, `ce off gpio40=0 i=4581`; LED stayed green/yellow. After settle-after-disable + `hold_disabled`: `ce parked gpio40=1 vbus=1 i=0`, `ce on gpio40=0 i=0`, `ce on gpio40=0 i=5702`, `ce off gpio40=1 i=5702`. STAT followed enable and park. `i=5702` is not a 555 mA charge-set reading. Default `embassy-debug` (no `charge`) restored after: `latched`, no `ce` lines, `imu=FaceUp`. Does not close `nyc-charge-stat`. |
| Deep sleep / GPIO6 wake | 2026-08-30. Default embassy-debug, USB-C down, `cargo xtask monitor` without `--acm-tty`. Short Page Down walked `shapes` → `legend` → `tones`. Hold 4 s: `scene=sleeping`, `embassy-debug: sleeping`, ROM `rst:0x5 (DSLEEP)`. 1 s hold after wake: `woke`, then `scene=tones` (same card). Early release: `woke` then `sleeping` with no `gt911` dance. Short press while asleep still resets the chip and the image re-sleeps without painting. Latch high; CH343 stayed enumerated. Closes `nyc-deep-sleep-wake`. Does not sit GPIO4 wake. Does not close `nyc-sleep-current`. |
