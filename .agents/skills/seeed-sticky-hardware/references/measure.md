...
| Operator UART session | Attended: `btn 4` / `5` / `6` on the three right-edge keys; USB-C unplug as host CH343 drop+return (firmware `vbus` edge usually lost); IMU `FaceUp` then `Landscape0` after tilt; GPIO7 stayed low with no edges; `gpio40=1` / `i=0` with `/CE` off after replug. GT911 `0x14` ACK. An earlier operator poll treated crate `NotReady` as failure and skipped status-clear; Bunny on glass does report points (100 kHz, 30 ms, tap on release). MicroSD not inserted. Restoring factory `app0` then returned stock. |
