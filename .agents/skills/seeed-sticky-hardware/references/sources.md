...
| [Seeed Hardware Overview](https://www.seeedstudio.com/sticky/docs/en/device-guide/hardware-overview/) | Official | Mechanics, 750 mAh, module list, IMU INT=GPIO7. Pin tables omit latch / charger / SD EN / mic EN |
| Official `Sticky_dashboard_demo` / `Sticky_peripheral_demo` zips ([catalog.md](catalog.md)) | Official (intent) | Published `pin_config.h`, latch hold-high, OTP `seeed_epaper`, GPIO39 low at boot. Sequences only — never electrical fact |
| [Seeed ESP-IDF Pages and Peripherals](https://www.seeedstudio.com/sticky/docs/en/device-guide/esp-pages/) | Official (intent) | I2C1 sensors vs I2C0 touch; SPI2 shared EPD+SD; unmount SD before refresh; unused SPI pins `-1` or GPIO0 is stolen; Battery “charging” is GPIO9 VBUS |
| [Seeed ESP-IDF Refresh and Low Power](https://www.seeedstudio.com/sticky/docs/en/device-guide/esp-refresh/) | Official (intent) | Gray4 / mono full / mono partial; partial is a full comparison frame; do not partial a gray4 page; AI-hold sleep; GPIO4 `ext1` ANY_LOW |
...
| FreeInk `STICKY` board profile | Third-party | Mic / SHT40 / GPIO40 *wiring intent*; GPIO39 left undriven (prefer this over official-demo / Bunny boot-enable for default debug images). 40 MHz / `NO_FLIP` still pending |
...
| GPIO39 `/CE` | Default images park high. Attended 2 s pulse then park on USB (same feature). Default `embassy-debug` restored after | Official dashboard `sticky_charger_init` and Bunny `board_charger_init` drive GPIO39 **low** at boot. FreeInk STICKY leaves it undriven. Do not copy official/Bunny boot-enable into a default debug image |
| GT911 | UART `touch n=5`, `st=0x85` after INT-low (Rev.09 §6.1 → `0x5D`). This FPC delivers 5 (Rev.09 §1). Sample is **480×800**; `to_screen` is glass (panel 180° undone); gray4 hit-test uses `to_framebuffer` + `gray4_touch_framebuffer` + `framebuffer_to_page` (portrait flip Y / page mirror X: 2026-09-08 `scene=targets` `p0=73,399` / `page=399,73` / `expect=80,80`). 2026-09-04 Wi-Fi START: `p0=679,189` / `imu=Portrait0` missed until that split. Landscape0 START uses `gray4_touch_framebuffer` (OTP 180° only; do not OR). 2026-09-04 `wifi_ap`: `page=362,53 hit=1` / `p0=362,426` then `state=active`; OR image also toggled the empty opposite side. Landscape180 survey on the tap-canvas image: opposite `hit=0` (`p0` y≈53); canvas `hit=1` (`p0` y≈399); `wifi_survey count=16`. 2026-09-05 USB-C left, OTP-180 image: visible SoftAP `page=339,402 hit=1` / `p0=339,77` then `state=active`; ghost `page=341,70 hit=0` / `p0=341,409`. | Official dashboard `begin(800, 480)` is a landscape **software map**; the driver still reads `0x8146` as `sensor=` and `mapCoord`s onto that canvas. `transform_touch_coordinate` then scales those mapped points with 800 as X — do not apply that to raw 480-wide `cx`. INT-high → `0x14` ACK; init Status-clear path stayed `st=0x00`. A raw map that treated the sample as 800×480 compressed the keys axis to `y≈195`. |
...
| ANT1 / radio | UART `wifi n=` and `ble n=` in one embassy-debug `--features radio` listen; `imu=` still running. Default-image walk (2026-09-04): `scene=wifi_survey` / `scene=wifi_ap`. SoftAP join / `GET /` host-verified (spare STA, DHCP `192.168.4.50`, JSON + UART `wifi_ap … clients=1` / `wifi_http req=1 path=/`). Host disconnect: no later `wifi_ap` decrement on that image. Later operator glass `clients=0` `http=2` after a host leave (no UART). | Schematic on-board ANT1, shared 2.4 GHz |
