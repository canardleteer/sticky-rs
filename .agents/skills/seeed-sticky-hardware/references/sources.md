...
| GT911 | UART `touch n=5`, `st=0x85` after INT-low (Rev.09 §6.1 → `0x5D`). This FPC delivers 5 (Rev.09 §1). Sample is **480×800**; `to_screen` is glass (panel 180° undone); gray4 hit-test uses `to_framebuffer` + `framebuffer_to_page`. 2026-09-04 Wi-Fi START: `p0=679,189` / `imu=Portrait0` missed until that split. Landscape0 START uses `gray4_touch_framebuffer` (OTP 180° only; do not OR). 2026-09-04 `wifi_ap`: `page=362,53 hit=1` / `p0=362,426` then `state=active`; OR image also toggled the empty opposite side. Landscape180 survey: opposite `hit=0` (`p0` y≈53); ink `hit=1` (`p0` y≈399); `wifi_survey count=16`. | Official dashboard `begin(800, 480)` is a landscape **software map**; the driver still reads `0x8146` as `sensor=` and `mapCoord`s onto that canvas. `transform_touch_coordinate` then scales those mapped points with 800 as X — do not apply that to raw 480-wide `cx`. INT-high → `0x14` ACK; init Status-clear path stayed `st=0x00`. A raw map that treated the sample as 800×480 compressed the keys axis to `y≈195`. |
