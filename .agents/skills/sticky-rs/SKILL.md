...
| `host/remote-debug-broker/` | Length-prefixed JSON over a Unix socket (`publish = true`). Owns one `Session`. `SpawnSpec` exe + argv; no clap, no UART |
...
`remote-debug-broker` is the Unix-socket process model. Board pins,
