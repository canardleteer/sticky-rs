...
| `xtask/` | Clap front-end at the repo root (`cargo xtask`). Maps flags to `sticky-host`; `repo_root()` is the parent of this package. `remote-debug --mcp` is an rmcp subtree, not the full CLI |
