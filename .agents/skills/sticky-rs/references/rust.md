...
7. GT911 from
   ...
   (`View::hit_raw` / `View::hit_framebuffer`); sit algebra stays in
   [touch.md](../../seeed-sticky-hardware/references/touch.md).
...
disconnect produced no later `wifi_ap` decrement on that
image. Later the same evening, operator glass showed
...
| SSD1677 | `ssd1677-gray4` (this repo) | Dual-plane four-gray; Sticky uses **OTP** (no default MCU LUT). First-time canvas: [draw-and-touch.md](../../seeed-sticky-hardware/references/draw-and-touch.md). 10 MHz SPI. Wait on BUSY with `embedded-hal-async` `Wait`, not a spin loop. Not crates.io `ssd1677` |
| GT911 | board `touch` + embassy-debug `Register` poll | Own EN/RST/INT + Sticky transform. First-time draw / hit-test: [draw-and-touch.md](../../seeed-sticky-hardware/references/draw-and-touch.md) (`View::hit_raw` / `View::hit_framebuffer`; `DigitizerSample` / `FramebufferPoint` / `GlassPoint` / `PagePoint` / `HitRect`). Mux GPIO41/42 off JTAG F0. `Register` / `Command` / `StatusWrite` / `StatusBits` / `StatusHeartbeat`. Crate `init()` writes `Command::ReadCoordinates` — embassy does not call it. On a physical unit: INT=0 → `PairBaBb`, `I2C_MAX_HZ`, `Register::Points` byte 0, **`touch n=5`**. `to_screen` takes the **480×800** sample (not panel 800×480); USB-down ink corners land on 800×480. Wi-Fi START/STOP hit-test uses `to_framebuffer` + `View::hit_framebuffer` (gray4 `set_gray` writes `(W-1-x, H-1-y)`; Landscape0 uses that OTP 180 only; `to_screen` already undoes the panel 180°). `STATUS_HEARTBEAT` is `EverySecs(10)` or `Off`. INT-high + init Status-clear stayed `st=0x00`. INT after reset: floating (`Pull::None`) |
...
| Wi-Fi / BLE | `esp-radio` or `esp-idf-svc` | Default embassy-debug `pair` + `wifi`: DisplayOnly advertise `sticky-rs` only on the pair card; idle-until-touch STA survey + WPA2 SoftAP (`sticky-rs-AP` / `sticky26`, `embassy-net` + `edge-dhcp`). UART `pair pin=` / `pair ok` / `wifi_survey` / `wifi_ap` / `wifi_http` (never a MAC / neighbor SSID). On a physical unit: host BlueZ Connect completed SMP. 2026-09-04 walk printed both Wi-Fi scene tokens; START that hit-tested `to_screen` as gray4 canvas produced no radio line; first-time recipe is [draw-and-touch.md](../../seeed-sticky-hardware/references/draw-and-touch.md) (`View::hit_framebuffer`, not UART `p0=`). Both landscape holds use `gray4_touch_framebuffer` OTP 180° only; portrait inverts the tap canvas; do not OR. Landscape sit (2026-09-04): `wifi tap page=362,53 hit=1` / `p0=362,426` then `wifi_ap state=active`; OR image also toggled the empty opposite side. 2026-09-05 USB-C left: visible SoftAP `page=339,402 hit=1` / `p0=339,77` then `state=active`; ghost `page=341,70 hit=0` / `p0=341,409`. SoftAP join / `GET /` is **host-verified** (spare STA, DHCP `192.168.4.50`, JSON `device` / `scene=wifi_ap` / `wifi` counts; UART `wifi_ap … clients=1` then `wifi_http req=1 path=/`). Host disconnect produced no later `wifi_ap` decrement on that image. Later operator glass `clients=0` `http=2` after a host leave (no UART). `--features radio` remains the exclusive scan sit. Do not BlueZ `Pair()` while `request_security()` is in flight. `trouble-host` 0.7 needs `central` for `GAP_SERVICE_ATTRIBUTE_COUNT` |
