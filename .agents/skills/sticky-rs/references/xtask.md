...
| `remote-debug` | live BLE; UART on auto-PIN | Encrypted GATT after DisplayOnly pair (`--features remote-debug` image). Advertise `sticky-rs` from splash on every boot (pair card optional). BlueZ **Connect**, not `Pair()`. A ConnectRPC owner (`$XDG_RUNTIME_DIR/sticky-rs/remote-debug.connect` loopback URI, one process) holds 0..N GATT `Session`s keyed by advertise name, never a MAC. `connect` starts a detached owner if needed (stdio to `$XDG_RUNTIME_DIR/sticky-rs/remote-debug.log` or `/run/user/<uid>/sticky-rs/` when `XDG_RUNTIME_DIR` is unset), returns `pairing`, and exits; poll `status` until `connected` or `pair failed`. CLI `connect --wait` polls in that process (~60 s); MCP `connect` has no `--wait`. GATT stays up after pair and after client hangup. `serve` is an optional foreground log (Ctrl-C shuts the owner down). Other leaves (`status`, `logs`, `list-targets`, inject, snapshot, `reboot`, `disconnect`) are RPC and refuse with `no broker; run connect or serve` plus the endpoint path when the file is missing. `disconnect` drops one session; an empty map (sit over) also `Shutdown`s the owner. Default `connect` scrapes a new UART `pair pin=` (UART lock only for that scrape; fails if `monitor` holds it). If inventory has no QinHeng TTY but usbfs still sees exactly one `1a86:55d3` (`cdc-acm` left detached after a listen skipped Drop), auto-PIN claims that unique device over usbfs instead of requiring a replug. That image reprints `pair pin=` every 5 s on splash or the pair card until `pair ok`. `--pin` skips UART. `--remember` allowlists factory / CH343 USB serial in gitignored `developer-data/remote-debug/` (never a MAC, never the PIN). After `pair ok` the image holds GATT when walking off the pair card. `reboot` software-resets the **embedded MCU** (not the host) and re-pairs unless `--no-reconnect`; leftover BlueZ LTK is forgotten. `--mcp` is the same client on **this subtree only** (not `flash-app` / restore); do not treat stdio MCP as the session owner. Do not also run `monitor` during auto-PIN. `inject-touch` is `FramebufferPoint` (pre-rotation 800×480), or `--page` page pixels (gray4 hit-test inverse), not UART `p0=` / `GlassPoint`. `--phase` `down` / `move` / `up` (unset = tap). `inject-button` is a short press (`--release` for the up edge); wait for LAST compose before `get-snapshot`. `logs` drains the Target / Scene ring. `status` repeats last-known `scene` / `hold` / expect from the last snapshot (stale after inject). CLI prints `scene=7(targets)`. Planes are `developer-data/remote-debug/snapshots/snap-<hex>.bw` / `.red` / page-space `.png`; the envelope also carries `scene` / `hold` / `target_step` / expect. BlueZ TX notify is a FIFO (`VecDeque`); a `Vec` + `pop` reversed multi-chunk snapshots (`frame: Version`). How-to and sit notes: [mcp-interactions.md](mcp-interactions.md) |
