...
| `build-fw` | no | Host-only. `cargo +esp build -p <fw> --profile release-fw --target xtensa-esp32s3-none-elf -Zbuild-std=core,alloc --locked` then `espflash save-image` (no port). IMAGE is `simple-debug` or `embassy-debug`. `--features operator` on simple-debug; `--features mic`, `radio`, `spi20`, or `sd` on embassy-debug. ELF and `.bin` under workspace `target/xtensa-esp32s3-none-elf/release-fw/` |
