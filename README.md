...
| `build-fw` | no | Host-only. `cargo +esp` + `save-image` for `simple-debug` or `embassy-debug`. Default embassy-debug includes `pair` + `wifi`. `--features operator` / `mic` / `radio` / `pair` / `wifi` / `spi20` / `sd` / `charge` |
