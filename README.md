...
| `build-fw` | no | Host-only. `cargo +esp` + `save-image` for `simple-debug` or `embassy-debug`. Default embassy-debug includes `pair`. `--features operator` / `mic` / `radio` / `pair` / `spi20` / `sd` / `charge` |
