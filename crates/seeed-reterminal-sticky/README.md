...
Every row is something the product is known to have. **This crate** means a
typed helper, pin constant, address, or rail. **Not in this crate** means the
board can do it, but you talk to a chip driver or the MCU HAL yourself.

### Power and charging

| Feature | This crate | Rest of the stack |
| --- | --- | --- |
| Stay-alive latch (`PWR_HOLD` GPIO45, `PWR_LOCK` GPIO46) | `Latch::acquire` / `Latch::release`; every rail constructor needs a `Latched` witness | Firmware supplies the two output pins. Releasing the latch is a software power-off on battery. |
| USB / external present | `pins::EXTERNAL_POWER_SENSE` (GPIO9, high = VBUS) | Input in the HAL. Divider / ADC use is unconfirmed. |
| BQ25616 charger `/CE` (active low) and status | Pin numbers only: `pins::CHARGE_EN`, `pins::CHARGE_STATUS` | [`bq25616`](https://github.com/canardleteer/sticky-rs/tree/main/crates/bq25616) owns `/CE` typestate. Status polarity on this board is **unmeasured**; do not interpret the pin as “charging”. |
| Dual-color charge LED (next to USB-C) | **Not in this crate** | Driven by the charger, not an MCU GPIO. |
| 750 mAh 1S pack / BQ27220 fuel gauge | I2C address `addresses::BQ27220` (`0x55`) on the sensor bus | [`bq27220`](https://github.com/canardleteer/sticky-rs/tree/main/crates/bq27220) on `pins::SENSOR_I2C_*`. Reads are safe; unseal / `CFGUPDATE` / FCC writes are opt-in. |

### Display and touch

| Feature | This crate | Rest of the stack |
| --- | --- | --- |
| 3.97" 800×480 mono e-paper (SSD1677, four-gray via dual planes + panel OTP) | `display`: geometry, `SPI_MAX_HZ` (10 MHz, mode 0), `RefreshKind::{Full, Partial, Gray4}`, `controller_config()` (OTP, `lut: None`) | [`ssd1677-gray4`](https://github.com/canardleteer/sticky-rs/tree/main/crates/ssd1677-gray4) for opcodes. Shared SPI with the card (`pins::SPI_SCLK` / `SPI_MOSI` / `SPI_MISO`, `pins::EPD_CS`). This crate does **not** ship a waveform LUT. |
| Panel 3.3 V rail | `EpdRail` on GPIO47; no unconditional `disable` | After the controller deep-sleep command, pass `PanelParked::after_deep_sleep_command()` into `disable_after_panel_sleep`. |
...
| Feature | This crate | Rest of the stack |
| --- | --- | --- |
| SHT40 humidity / temperature | `addresses::SHT40` (`0x44`) | [`sht4x`](https://crates.io/crates/sht4x) |
| PCF8563 real-time clock | `addresses::PCF8563` (`0x51`) | [`pcf8563-dd`](https://crates.io/crates/pcf8563-dd) (register VL/integrity flag still wants a datasheet spot-check) |
| LSM6DS3TR-C IMU | `addresses::LSM6DS3TRC` (`0x6A`); `imu::classify` maps a raw sample onto this enclosure | [`lsm6ds3tr`](https://crates.io/crates/lsm6ds3tr) over I2C. Do not drive GPIO7 as output (see below). |

### Audio, storage, UI, debug

| Feature | This crate | Rest of the stack |
| --- | --- | --- |
| PDM microphone | **Power and pins only.** `MicRail` on `pins::MIC_POWER_EN` (GPIO38); clock `pins::MIC_CLK` (GPIO19), data `pins::MIC_DATA` (GPIO20). Settle time is unmeasured. There is **no** PDM/I2S driver, buffer, or sample API here. | Enable the rail, then attach those GPIOs to the ESP32-S3 PDM RX. GPIO19/20 are also the USB-Serial-JTAG pads: after deep sleep, disable that pad before attaching I2S or the mic stays mute. Hold `MIC_POWER_EN` low when unused and in sleep. No loudspeaker on this board. |
...
These are real product capabilities. This crate does not wrap them:
...
There is no `Microphone` type and no sample stream. The board has no
loudspeaker. Access is:

1. `Latch::acquire`, then `MicRail::new` on GPIO38 and `enable`.
2. After **deep-sleep wake**, disable the ESP32-S3 USB-Serial-JTAG pad on
   GPIO19/20 *before* attaching I2S. Those pins are the USB D−/D+ pads; the
   pad function reclaims them on wake even though USB-C debug is the CH343P
   on UART0. Cycle GPIO38 low briefly, then enable, so the capsule is not
   left half-powered.
3. Map `pins::MIC_CLK` and `pins::MIC_DATA` to PDM RX (working community
   firmware uses 16 kHz, 16-bit, left channel).
4. `disable` the rail (or hold the pin low) when you are not recording and
   across sleep.
