...
/// Sensor I2C clock. **Also a strapping pin** — never put it on the SPI bus.
...
///
/// **Also a strapping pin** (ESP32-S3 v2.2 §3.4, JTAG source). Floating at
/// reset; ordinary IO after strapping hold time. Never put it on the SPI bus.
...
/// community app. **Input only** — see [`crate::AMBIGUOUS_INTERRUPT_NOTE`].
...
/// MicroSD card detect. Use a pull-up; polarity unmeasured.
...
///
/// ESP32-S3 v2.2 Table 2-1: no default pull at or after reset.
...
/// BQ25616 charge status. Polarity unmeasured; do not interpret it.
pub const CHARGE_STATUS: u8 = 40;

/// GT911 reset.
///
/// Default IO MUX function is JTAG `MTDI` (v2.2 Table 2-4). Firmware must
/// mux this pad to GPIO before the address-select dance.
pub const TOUCH_RST: u8 = 41;
/// Touch power enable, active high. ~250 ms settle.
///
/// Default IO MUX function is JTAG `MTMS` (v2.2 Table 2-4). Mux to GPIO.
...
/// (VDD_SPI voltage); default weak pull-down at reset.
