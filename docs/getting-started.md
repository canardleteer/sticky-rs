...
| `host/remote-debug-broker/` | ConnectRPC owner that holds 0..N GATT `Session`s (`SpawnSpec`; no clap, no UART) |
| `protos/` | Shared remote-debug IDL (`buf lint` STANDARD+COMMENTS) |
