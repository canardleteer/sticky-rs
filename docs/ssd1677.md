...
not a second Deep Sleep opcode. RAM stays. The 2048 copy of
`seeed_epaper` omits those vtable slots. Stock UC8179 leaves
them NULL (`driver standby not supported`). The crate exposes
`Ssd1677::standby` and `Ssd1677::resume` on `Active` for those
bytes. Deep sleep remains `Ssd1677::sleep` → `Asleep`. Do not
invent a third `0x10` mode.
...
standby (0x22 = 0x03) / resume (0x22 = 0xC0), or
