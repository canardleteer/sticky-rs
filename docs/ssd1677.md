...
Schematic Rev 01 names the analog rails on the 24-pin FPC: **VGH**,
**VGL**, **VSH1**, **VSH2**, **VCOM**, **VPP**, and `EP_3V3`. The glass
part number is still open (`nyc-panel-glass` in the hardware skill).
Named rails still do not produce a 105-byte `0x32` table, and they are
not permission to uncomment one.
...
3. **SPI at 10 MHz, mode 0.** The controller’s write max is 20 MHz.
   Embassy-debug `--features spi20` painted a clean splash at 20 MHz.
   `--features sd` identified the card at 400 kHz and ACKed status at
   10 MHz and 20 MHz. Keep the default at 10 MHz. 40 MHz profiles exist
   for other products and are out of spec on this shared bus.
...
6. **Write both RAM planes**, then pick a sequence:
   - **Full** black/white: `UpdateSequence::DISPLAY_MODE_1_WITH_TEMP`
     (`0xF7`), then `0x20`. Cold boot: white clear, because the last frame
     survives reset.
   - **Partial** black/white: `UpdateSequence::DISPLAY_MODE_2_WITH_TEMP`
     (`0xFF`), then `0x20`. Still a **panel-wide** waveform. Unchanged
     pixels stay put only if previous and new RAM match.
   ...
   - **Four-gray:** `SEEED_GRAY4_TEMPERATURE` (`0x1A`, opaque panel
     ...
     (`0xD7`), then `0x20`.
     `SEEED_GRAY4` is Seeed/stock; it is not a named row in the Table 7-1
     excerpt we extracted.
7. **Wait while BUSY is high** (seconds is normal). Do not send another
   command. `Master Activation` must finish.
8. **Deep sleep `0x10` = `DEEP_SLEEP_ENTER` (`0x03`)**, wait ~100 ms, then
   drop the rail. Datasheet Table 7-1 enters deep sleep only when those two
   bits are `11`. `0x01` is a
   no-op for sleep (Lotus/bb_epaper use it as “keep RAM”; this crate does not).
   BUSY stays high in deep sleep; that is not a hang.

Orientation on a physical unit: draw 800×480, `rotate180_gray4`,
controller `mirror_x`.
...
| SPI | 10 MHz default, mode 0, BUSY active high | On a physical unit at 10 MHz and 20 MHz (card CS high); datasheet max 20 MHz |
...
| Deep sleep `0x10` | `DEEP_SLEEP_ENTER` (`0x03`), then ~100 ms | Table 7-1 `A[1:0]=11`; Seeed / stock `ssd1677_sleep` |
| Stock standby `0x22` | `UpdateSequence::DISABLE_ANALOG_AND_CLOCK` (`0x03`) then `0x20` | Table 7-1 disable analog+clock; stock `ssd1677_standby` |
| Stock resume `0x22` | `UpdateSequence::ENABLE_CLOCK_AND_ANALOG` (`0xC0`) then `0x20` | Table 7-1 enable clock+analog; stock `ssd1677_resume` |
...
Standby vs sleep: stock `ssd1677_standby` / `ssd1677_resume` are
`0x22` + `0x20` power-down / power-up sequences (see the table),
not a second Deep Sleep opcode. The crate exposes
`Ssd1677::standby` and `Ssd1677::resume` on `Active` for those
bytes. Deep sleep remains `Ssd1677::sleep` → `Asleep`. Do not
invent a third `0x10` mode. The 2048 copy of `seeed_epaper`
omits those vtable slots. Stock UC8179 leaves them NULL
(`driver standby not supported`).

On this Sticky, after `standby()` BUSY stayed high (clock off).
Stock `resume()` (`0xC0` then `0x20`) and Table 7-1
`ENABLE_CLOCK` (`0x80` then `0x20`) left `busy=1`. embassy-debug
then pulses RST, runs OTP `init`, and redraws the same card
(UART `epd resume rst`, then `resume`, then the same `scene=`).
That path loses controller RAM. Do not wait a full refresh
timeout after `standby()` or a failed `resume()`. We have not
proven that stock 1.1.0 `ssd1677_resume` also pulses RST; we
only extracted the `0x22` / `0x20` bytes.
...
standby (`UpdateSequence::STANDBY`); on this glass `RESUME` does
not drop BUSY — RST + init + redraw — or sleep
(`Command::DeepSleepMode` + `DeepSleep::Enter`) →
wait SLEEP_HOLD_MS → drop EPD_EN
```

Gray4: split with `PlaneMapping::SEEED_OTP`, write `0x24` then `0x26`, write
`SEEED_GRAY4_TEMPERATURE`, then `UpdateSequence::SEEED_GRAY4`. Do **not**
mix `SEEED_OTP` with a microcontroller `0x32` table, or `LUT_INDEX_ORDER`
with OTP gray4 — they are opposite polarities.
...
| Glass part number vs temperature bands | measurement backlog `nyc-panel-glass` | Schematic names analog rails; no glass PN. Toolbox **GDEM0397T81P**, GxEPD2 `GDEM0397T81`, and Good Display product 613 (**GDEY0397T81P** sheet) are hints, not a BOM |
...
| **VCOM / VGH / VGL / VSH1 / VSH2 / VPP** | Analog rails named on schematic Rev 01. OTP brings them up on Sticky; do not write `0x03`/`0x04`/`0x2C`. |
