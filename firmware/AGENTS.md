...
| `embassy-debug/` | `esp-hal` + Embassy | Member. Panel always on. Default image is keys / glass / IMU / splash. `mic` / `radio` / `pair` / `sd` / `charge` / `spi20` are opt-in and exclusive where the package `AGENTS.md` says so |
